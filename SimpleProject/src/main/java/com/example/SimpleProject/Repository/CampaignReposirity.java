...
public interface CampaignReposirity extends JpaRepository<Campaign, Integer> {
    ...
    @Query(value = "select campaign.created_time as createdTime, phone_list.status, phone_list.phone, phone_list.retry, phone_list.service_id as serviceId from campaign inner join phone_list ON email_list.campaign_id = ?1 and campaign.id = ?1 ", nativeQuery = true)
